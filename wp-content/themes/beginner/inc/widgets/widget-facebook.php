...
class Beginner_Facebook_Widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'facebook-likebox',
			apply_filters( 'beginner_widget_name', esc_html__( '&raquo; Facebook Like Box', 'beginner' ) ),
			array(
				'classname' => 'widget_facebook_likebox',
				'description' => __( 'Display a Facebook Like Box to connect visitors to your Facebook Page', 'beginner' )
			)
		);
	}
	...
	function widget( $args, $instance ) {
		...
		if ( empty( $like_args['href'] ) || ! $this->is_valid_facebook_url( $like_args['href'] ) ) {
			if ( current_user_can('edit_theme_options') ) {
				echo $before_widget;
				echo '<p>' . sprintf( __( 'It looks like your Facebook URL is incorrectly configured. Please check it in your <a href="%s">widget settings</a>.', 'beginner' ), admin_url( 'widgets.php' ) ) . '</p>';
				echo $after_widget;
			}
		...
		$like_bg_colour          = apply_filters( 'beginner_fb_likebox_bg', ( 'dark' == $like_args['colorscheme'] ? '#000' : '#fff' ), $like_args['colorscheme'] );
		...
		if ( ! empty( $title ) ) :
			...
			echo apply_filters( 'beginner_facebook_likebox_title', $likebox_widget_title, $title, $page_url );
		...
		do_action( 'beginner_stats_extra', 'widget', 'facebook-likebox' );
	}
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'href' ); ?>">
				<?php _e( 'Facebook Page URL', 'beginner' ); ?>
				...
				<br />
				<small><?php _e( 'The Like Box only works with <a href="http://www.facebook.com/help/?faq=174987089221178">Facebook Pages</a>.', 'beginner' ); ?></small>
			</label>
		</p>

		<p>
			<label for="<?php echo $this->get_field_id( 'width' ); ?>">
				<?php _e( 'Width', 'beginner' ); ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'colorscheme' ); ?>">
				<?php _e( 'Color Scheme', 'beginner' ); ?>
				<select name="<?php echo $this->get_field_name( 'colorscheme' ); ?>" id="<?php echo $this->get_field_id( 'colorscheme' ); ?>">
					<option value="light" <?php selected( $like_args['colorscheme'], 'light' ); ?>><?php _e( 'Light', 'beginner' ); ?></option>
					<option value="dark" <?php selected( $like_args['colorscheme'], 'dark' ); ?>><?php _e( 'Dark', 'beginner' ); ?></option>
				</select>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'show_faces' ); ?>">
				...
				<?php _e( 'Show Faces', 'beginner' ); ?>
				<br />
				<small><?php _e( 'Show profile photos in the plugin.', 'beginner' ); ?></small>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'stream' ); ?>">
				...
				<?php _e( 'Show Stream', 'beginner' ); ?>
				<br />
				<small><?php _e( 'Show the profile stream for the public profile.', 'beginner' ); ?></small>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'show_border' ); ?>">
				...
				<?php _e( 'Show Border', 'beginner' ); ?>
				<br />
				<small><?php _e( 'Show a border around the plugin.', 'beginner' ); ?></small>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'force_wall' ); ?>">
				...
				<?php _e( 'Show Wall', 'beginner' ); ?>
				<br />
				<small><?php _e( 'Show the wall for a Places page rather than friend activity.', 'beginner' ); ?></small>
			</label>
		</p>
	...
	function get_default_args() {
		...
		return apply_filters( 'beginner_facebook_likebox_defaults', $defaults );
	}
