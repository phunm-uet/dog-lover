 ...
 * @since      1.0.0
 */
class Beginner_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-beginner-recent widget_posts_thumbnail',
			'description' => __( 'Display recent posts with thumbnails.', 'beginner' )
		);

		// Create the widget.
		parent::__construct(
			'beginner-recent',                                   // $this->id_base
			__( '&raquo; Recent Posts Thumbnails', 'beginner' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $recent = get_transient( 'beginner_recent_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'beginner_recent_widget_' . $this->id, $recent );
	...
	function flush_widget_transient() {
		delete_transient( 'beginner_recent_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Recent Posts', 'beginner' ),
			'limit'     => 5,
			'show_date' => false
		);
