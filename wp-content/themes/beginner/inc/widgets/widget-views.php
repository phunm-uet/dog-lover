 ...
 * @since      1.0.0
 */
class Beginner_Views_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-beginner-views widget_posts_thumbnail',
			'description' => __( 'Display the most views posts with thumbnails.', 'beginner' )
		);

		// Create the widget.
		parent::__construct(
			'beginner-views',                                        // $this->id_base
			__( '&raquo; Most Views Posts Thumbnails', 'beginner' ), // $this->name
					...
					echo '<li>';
						...
						echo '<div class="entry-meta">';
							...
							if ( $instance['show_view'] && function_exists( 'entry_views_get' ) ) :
								echo ' <span class="entry-view">- ' .  entry_views_get( array( 'after' => __( ' Views', 'beginner' ) ) ) . '</span>';
							endif;
						echo '</div>';
					echo '</li>';
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Most Views Posts', 'beginner' ),
			'limit'     => 5,
			'show_date' => false,
			'show_view' => false
		);
	...
	?>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'show_view' ); ?>">
				<?php _e( 'Display post view?', 'beginner' ); ?>
			</label>
		</p>
