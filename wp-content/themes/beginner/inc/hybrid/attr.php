...
function hybrid_attr_sidebar( $attr, $context ) {
	...
	if ( $context ) {
		...
		if ( $sidebar_name ) {
			...
			$attr['aria-label'] = esc_attr( sprintf( _x( '%s Sidebar', 'sidebar aria label', 'beginner' ), $sidebar_name ) );
		}
	}
...
function hybrid_attr_menu( $attr, $context ) {
	...
	if ( $context ) {
		...
		if ( $menu_name ) {
			...
			$attr['aria-label'] = esc_attr( sprintf( _x( '%s Menu', 'nav menu aria label', 'beginner' ), $menu_name ) );
		}
	}
...
function hybrid_attr_entry_published( $attr ) {
	...
	$attr['content'] = get_the_time( 'Y-m-d\TH:i:sP' );
	$attr['itemprop'] = 'datePublished';

	// Translators: Post date/time "title" attribute.
	$attr['title']    = get_the_time( _x( 'l, F j, Y, g:i a', 'post time format', 'beginner' ) );
...
function hybrid_attr_comment_published( $attr ) {
	...
	$attr['title']    = get_comment_time( _x( 'l, F j, Y, g:i a', 'comment time format', 'beginner' ) );
	$attr['itemprop'] = 'datePublished';
 ...
 * @return array
 */
function beginner_entry_markup( $classes ) {
...
add_filter( 'post_class', 'beginner_entry_markup' );
 ...
 * @return string
 */
function beginner_comments_popup_link_attributes( $attr ) {
	return ' itemprop="discussionURL"';
}
add_filter( 'comments_popup_link_attributes', 'beginner_comments_popup_link_attributes', 5 );
